...
def boom(row, col, buf, bomb_type):
    if bomb_type == COL:
        ...
        buf[row][col] = 2
        if row > 0: buf[row - 1][col] = 2
        if row < n - 1: buf[row + 1][col] = 2
        if col > 0: buf[row][col - 1] = 2
        if col < n - 1: buf[row][col + 1] = 2
    else:
        buf[row][col] = 2
        if row > 0 and col > 0: buf[row - 1][col - 1] = 2
        if row > 0 and col < n - 1: buf[row - 1][col + 1] = 2
        if row < n - 1 and col > 0: buf[row + 1][col - 1] = 2
        if row < n - 1 and col < n - 1: buf[row + 1][col + 1] = 2
